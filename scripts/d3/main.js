...
function initArxivDataCleanedChart() {
  const svg = d3.select('.arxiv-data-cleaned-chart')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
  ...
  const categorysSvg = svg.append('g')
  ...
  d3.json('./arxiv-data-cleaned.json').then(arxivData => {
    ...
    const colorsXscale = d3.scaleOrdinal()
      .domain(categorys)
    ...
    const xScale = d3.scaleLinear()
      .domain(d3.extent(arxivData, function (d) {
    ...
    chartSvg.selectAll('circle')
      ...
      .on('mouseover',function(d) {	
        ...
        <p><span>Published:</span> ${d3.timeFormat("%Y-%m-%d %H:%M:%S")(formatTimeToUnix(d.published))}</p>
        `
        tooltip.html(html)
    ...
    const categoryG = categorysSvg.selectAll('g')
      .data(categorys)
      .enter()
      .append('g')
